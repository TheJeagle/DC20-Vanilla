...
function createActionCard(action, creatureContext, options = {}) {
  ...
  const saveDC = stats.saveDC ?? creatureContext?.saveDC ?? 0;
  const baseDamage = Number(stats.damage) || 0;
  return createActionCardElement(action, saveDC, baseDamage, { showTrigger: options?.showTrigger ?? false });
}
