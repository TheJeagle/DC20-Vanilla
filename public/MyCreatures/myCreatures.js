...
async function annotateCreatureLikes(user, creatures) {
  if (!user || !Array.isArray(creatures) || creatures.length === 0) return;
  ...
  await Promise.all(
    creatures.map(async (creature) => {
      ...
      try {
        ...
        console.warn('Failed to resolve like status', error);
...
function updateLikeButtonAppearance(button, creature) {
  ...
  button.setAttribute('aria-label', isLiked ? `Unlike (${likes} likes)` : `Like (${likes} likes)`);
...
function createLikeButton(creature) {
  ...
  updateLikeButtonAppearance(button, creature);
  button.addEventListener('click', () => {
    if (button.disabled) return;
    handleToggleLike(creature, button).catch((error) => {
      console.error('Failed to toggle like', error);
      setStatus('error', 'Could not update likes. Please try again.');
    });
  });
  return button;
}
...
function createCreatureCard(creature, featuresById) {
  ...
  openButton.addEventListener('click', () => {
    window.location.href = `../CreateCreature/createCreature.html?creatureId=${encodeURIComponent(creature.id)}`;
  });
  ...
  actions.append(likeButton, openButton, deleteButton);
...
async function loadUserCreatures(user, { showLoading = true } = {}) {
  ...
  try {
    ...
    currentCreatures = snapshot.docs.map((docSnap) => {
    ...
    await annotateCreatureLikes(user, currentCreatures);
...
function renderCreatureTable(creatures, featuresById) {
  ...
  creatures.forEach((creature) => {
    ...
    const cells = [
      ...
      resolveLikeCount(creature.totalLikes),
    ];
